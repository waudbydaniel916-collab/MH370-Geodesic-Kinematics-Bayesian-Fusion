...
The framework integrates deep-sea sound propagation dynamics over the SOFAR channel ($V_{\text{sound}} = 1482.0\text{ m/s}$) to calculate precise travel-time vectors from the pinpointed Bayesian crash center. This module actively filters out macro-seismic geological events to enable micro-signal background auditing:
...
## Kinematic Glide Extension Trajectory Engine
The framework incorporates an aerodynamic descent simulation module to map potential aircraft trajectories following fuel exhaustion. By modeling a Boeing 777 airframe's unpowered glide profile ($16.0:1\text{ Lift-to-Drag ratio}$) starting from a flameout cruise altitude of $38,000\text{ feet}$, the solver projects a maximum sliding range footprint of **185.32 km** centered on the recovered flight simulator coordinates:

* **North Boundary Limit**: $-43.3322^\circ\text{S}, 104.0000^\circ\text{E}$
* **East Boundary Limit**: $-44.9758^\circ\text{S}, 106.3497^\circ\text{E}$
